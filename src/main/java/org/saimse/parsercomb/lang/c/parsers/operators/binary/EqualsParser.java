...
public class EqualsParser implements Parser<Expression> {
    @java.lang.Override
    public Pair<String, Expression> parse(String input) throws BadParseException {
        Pair<String, Pair<Expression, Expression>> op = new And<>(new LeftToken<>(new Or<>(new MemberAccessParser(), new MemberArrowAccessParser(), new ExpressionWithPrimaryParser()), new Left<>(new StringParser("==")
                , new WhitespaceParser())), new Left<>(new ExpressionWithPrimaryParser(), new WhitespaceParser())).parse(input);
        return new Pair<>(op.a, new BinaryOp(BinaryOp.OP.EQ, op.b.a, op.b.b));
    }
}
